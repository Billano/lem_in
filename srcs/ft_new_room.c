...
/*   Updated: 2017/06/27 01:34:40 by eurodrig         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/ft_lem_in.h"

t_room	*ft_new_room(t_list_s *the_rooms, char *src, t_room_list room_list,\
		int i)
{
	t_room	*room;
	char	**info;
	...
	while (the_rooms)
	{
		...
		if (!ft_strcmp(info[0], src))
		{
			...
			room->next = 0;
			room->weight = ft_room_weight(room_list.x, room_list.y, room->x,\
					room->y);
